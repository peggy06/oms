...
@section('content')
    <ul class="list-unstyled">
        @if($permissions->where(['accepted' => 0, 'to' => auth()->user()->id])->count() == 0)
            ...
            @foreach($sorted_permission as $permit)
                <li style="padding: 5px 15px 0px 15px;">
                    ...
                    <div class="text-right">
                        <a href="{{ route('adviserRequestAccept', encrypt($permit['id'])) }}" class="btn btn-success btn-sm">Accept</a>
                        <a href="{{ route('adviserRequestReject', encrypt($permit['id'])) }}" class="btn btn-default btn-sm">Reject</a>
                    </div>
                </li>
                <hr>
            @endforeach
