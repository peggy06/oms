    ...
    <div style="background-color: #5e5e5e">
        <div class="legal margin-default">
            ...
            <p style="font-size: 12pt;line-height: 2">
                <b>Name:</b> {{ $user->lastname}}, {{ $user->firstname }}<br>
                <b>Contact:</b> 0{{ $user->profile->contact }}<br>
                <b>Name and Contact no. in case of emergency:</b> ________________________ <br>
                <b>Year and Section:</b> {{ $course->find($users->find($user->id)->profile->company_id)->code }} <br>
                <b>Adviser/OJT Coordinator:</b> {{ $users->find($user->under_to)->name }}
            </p><br>
            <h4><b>Company Information</b></h4>
            <p style="font-size: 12pt;line-height: 2">
                <b>Name:</b> {{ $company->name }}<br>
                <b>Contact:</b> 0{{ $user->profile->company_contact }}<br>
